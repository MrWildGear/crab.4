"""
Test file for EVELogReader.export_logs method
Tests the log export functionality
"""
import unittest
...
class TestExportLogs(unittest.TestCase):
    ...
    def test_export_logs_handles_exception(self):
        """Test that export_logs handles exceptions gracefully"""
        with patch('eve_log_reader.EVELogReader.find_eve_log_directory') as mock_find_dir, \
             patch('eve_log_reader.EVELogReader.setup_logging'), \
             patch('eve_log_reader.EVELogReader.setup_ui'), \
             patch('eve_log_reader.EVELogReader.load_log_files'), \

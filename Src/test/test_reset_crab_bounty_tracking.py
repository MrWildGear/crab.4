"""
Test file for EVELogReader.reset_crab_bounty_tracking method
Tests the CRAB bounty tracking reset functionality
"""
import unittest
...
class TestResetCrabBountyTracking(unittest.TestCase):
    def setUp(self):
    ...
    def test_reset_crab_bounty_tracking_clears_data(self):
        """Test that reset_crab_bounty_tracking clears all CRAB bounty data"""
        with patch('eve_log_reader.EVELogReader.find_eve_log_directory') as mock_find_dir, \
            ...
            # Add some CRAB bounty data first
            reader.add_crab_bounty_entry("2024-01-01 12:00:00", 1000000, "test.log")
            reader.add_crab_bounty_entry("2024-01-01 12:01:00", 500000, "test.log")
            ...
            reader.reset_crab_bounty_tracking()
            
            # Verify data was cleared
            self.assertEqual(len(reader.crab_bounty_entries), 0)
            self.assertEqual(reader.crab_total_bounty_isk, 0)
            
    def test_reset_crab_bounty_tracking_empty_data(self):
        """Test reset_crab_bounty_tracking with empty data"""
        with patch('eve_log_reader.EVELogReader.find_eve_log_directory') as mock_find_dir, \
             patch('eve_log_reader.EVELogReader.setup_logging'), \
             patch('eve_log_reader.EVELogReader.setup_ui'), \
             patch('eve_log_reader.EVELogReader.load_log_files'), \
    ...
    def test_reset_crab_bounty_tracking_handles_exception(self):
        """Test that reset_crab_bounty_tracking handles exceptions gracefully"""
        with patch('eve_log_reader.EVELogReader.find_eve_log_directory') as mock_find_dir, \
             patch('eve_log_reader.EVELogReader.setup_logging'), \
             patch('eve_log_reader.EVELogReader.setup_ui'), \
             patch('eve_log_reader.EVELogReader.load_log_files'), \

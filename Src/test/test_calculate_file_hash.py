"""
Test file for EVELogReader.calculate_file_hash method
Tests the file hash calculation functionality
"""
import unittest
...
class TestCalculateFileHash(unittest.TestCase):
    ...
    def test_calculate_file_hash_valid_file(self):
        """Test calculating hash for valid file"""
        with patch('eve_log_reader.EVELogReader.find_eve_log_directory') as mock_find_dir, \
            ...
            file_path = "test.log"
            result = reader.calculate_file_hash(file_path)
    ...
    def test_calculate_file_hash_none_input(self):
        """Test calculating hash for None input"""
        with patch('eve_log_reader.EVELogReader.find_eve_log_directory') as mock_find_dir, \
             patch('eve_log_reader.EVELogReader.setup_logging'), \
             patch('eve_log_reader.EVELogReader.setup_ui'), \
             patch('eve_log_reader.EVELogReader.load_log_files'), \
            ...
            result = reader.calculate_file_hash(None)
            
            # Should return None for invalid input
            self.assertIsNone(result)
            
    def test_calculate_file_hash_consistent_output(self):
        """Test that calculate_file_hash produces consistent output for same file"""
        with patch('eve_log_reader.EVELogReader.find_eve_log_directory') as mock_find_dir, \
             patch('eve_log_reader.EVELogReader.setup_logging'), \
             patch('eve_log_reader.EVELogReader.setup_ui'), \
             patch('eve_log_reader.EVELogReader.load_log_files'), \
            ...
            file_path = "test.log"
            result1 = reader.calculate_file_hash(file_path)
            result2 = reader.calculate_file_hash(file_path)
